  ...
  return new Promise((resolve, reject) => {
    ...
    proc.on("close", (code) => {
      if (code === 0) resolve();
        ...
        reject(
          new Error(`npm install failed (code ${code}): ${stderr.slice(-200)}`),
        );
    });
  ...
  return new Promise((resolve, reject) => {
    const check = () => {
      if (Date.now() - start > timeoutMs) {
        return reject(
          new Error(`Timeout waiting for ${url} after ${timeoutMs / 1000}s`),
        );
      }
...
async function main() {
  ...
  for (const project of projects) {
    ...
    try {
      // Install if needed
      if (
        project.needsInstall &&
        !existsSync(path.join(project.dir, "node_modules"))
      ) {
